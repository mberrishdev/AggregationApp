...
namespace AggregationApp.Persistence.UnitOfWork
{
    public class EfCoreUnitOfWorkScope : IUnitOfWorkScope
    {
        ...
        {
            if (IsCompleted)
                throw new Exception("Unit of work scope is already completed");
            if (_index == 0)
        ...
        {
            if (IsRolledBack)
                throw new Exception("Unit of work scope is rolled back");
